...
#SBATCH --output=/simurgh/u/juze/regen_logs/fit_skel_%A.out
#SBATCH --error=/simurgh/u/juze/regen_logs/fit_skel_%A.err
#SBATCH --exclude=simurgh2

# 28-view mvbodyfit with SKELETAL-FK fast refine_RT.
# Same inputs / 28 subs / chunk size as sbatch_fit_28view.sh, but uses the
# new mhr_simplified_skel pipeline (precompute_kbody + cached k3d loss).
# Usage: sbatch sbatch_fit_simplified_skel.sh <seq>

set -u
SEQ=${1:?seq required}
echo "=== Job $SLURM_JOB_ID on $SLURMD_NODENAME — fit_skel $SEQ ==="
nvidia-smi -L
...
OUT_DIR=/simurgh2/datasets/HOI-M3/mhr_simplified_skel/$SEQ
...
$PY -u apps/mocap/run.py \
    --cfg config/hoim3_mhr_simplified_skel_refined.yml \
    --sequence "$SEQ" \
    --subs $SUBS \
    --out "$OUT_DIR" \
    --skip_vis
rc=$?
dt=$(($(date +%s) - t0))
echo "$(date) [$SEQ] fit_skel rc=$rc in ${dt}s"
exit $rc
