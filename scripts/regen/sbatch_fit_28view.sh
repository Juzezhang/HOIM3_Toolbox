...
#SBATCH --output=/simurgh/u/juze/regen_logs/fit28_%A.out
#SBATCH --error=/simurgh/u/juze/regen_logs/fit28_%A.err
#SBATCH --exclude=simurgh2

# 28-view mvbodyfit (no v33) with vit-conf=1 fallback for missing vitpose.
# Usage: sbatch sbatch_fit_28view.sh <seq>

set -u
SEQ=${1:?seq required}
echo "=== Job $SLURM_JOB_ID on $SLURMD_NODENAME — fit28 $SEQ ==="
nvidia-smi -L
...
rc=$?
dt=$(($(date +%s) - t0))
echo "$(date) [$SEQ] fit28 rc=$rc in ${dt}s"
exit $rc
