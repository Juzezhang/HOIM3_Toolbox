...
#SBATCH --output=/simurgh/u/juze/regen_logs/mono_v28_%A.out
#SBATCH --error=/simurgh/u/juze/regen_logs/mono_v28_%A.err
#SBATCH --exclude=simurgh2

# Run SAM-3D-Body mono MHR inference on HOI-M3 view 28 for one sequence.
...
# Usage: sbatch sbatch_mono_view28.sh <sequence_name>
...
EXISTING_NPZ=$(/usr/bin/find "$NFS_MIRROR/$SEQ/28/person0" -maxdepth 1 -name '*.npz' 2>/dev/null | head -1001 | wc -l)
if [ "$EXISTING_NPZ" -ge 1000 ]; then
  OUTPUT_ROOT=$NFS_MIRROR
  USE_LOCAL=0
  echo "[output] NFS already has $EXISTING_NPZ npz for v28 — skipping seed, using NFS directly"
...
$PY $SCRIPT --checkpoint_path $CKPT --mhr_path $MHR \
    --inference_type body --views "28" --root_path "$ROOT" \
...
rc=$?
dt=$(($(date +%s) - t0))
echo "$(date) [$SEQ] mono v28 finished rc=$rc in ${dt}s"
